...
## 6. Interactive text taxonomy (four species — complete specs)

*Every tappable text in Mineral is exactly one of these four. No screen invents a fifth. Implement the three link species as components in `components/Links.tsx`; the CTA is its own component (exists as the encounter affordance — tokenize it to match this spec).*

### 6a. The CTA — the doorway (one per surface, maximum)
The single element on a surface allowed to invite. Currently: the encounter affordance on Origin; the Begin pill on the Threshold.
- Container: full-width pill, `borderRadius: 999`, background `rgba(244,240,250,0.97)` (near-white on the dark ground), vertical padding 16, horizontal 24.
- Eyebrow: Inter 600 · 11 · UPPERCASE · tracking 1.6 · `rgba(10,5,16,0.55)`.
- Title: `serifSmall`-class Cormorant 500i · 18 · `#0a0510`.
- Arrow `→` at 18 · `rgba(10,5,16,0.6)`; in the informational/complete state the arrow drops to opacity 0.25 and the press is a quiet acknowledgment, not navigation.
- States: TODAY / CONTINUE / COMPLETE · TOMORROW (eyebrow carries the state; title carries the encounter).

### 6b. `linkPrimary` — the action
The screen's forward motion, where a full CTA pill would be too loud (block screens, sheets, forms).
- Inter 600 · 12 · UPPERCASE · tracking 1.8 · ` →` always · color `textPrimary` (phase text-tint permitted where an accent is wanted) · no underline.
- Press state: opacity 0.5. Tap target ≥ 44pt.
- Examples: `WHEN YOU'RE READY →`, `KEEP THIS →`, `RETURN TO THE MAP →`, `ENTER →`, `ALLOW`.

### 6c. `linkWhisper` — the offered doorway
Optional depth inside held/ritual surfaces (map, reading sheets, encounter flow).
- Inter 400 · 12 · lowercase · tracking +0.5 · always ends ` →` · **never underlined** · color `rgba(200,190,225,0.55)` (ritual dim-lavender — deliberately from the map palette, not the text-hierarchy tokens).
- Press state: opacity 0.35. Tap target ≥ 44pt via padding/hitSlop regardless of visual size.
- Examples: `four companions of this moment →`, `there's more here, if you have time →`, `the whole field lives in notes →`, `keep what comes →`, `add your birth date to see your spiral →`, `what you've walked through →`.

### 6d. `linkSecondary` — the choice
Choices, dismissals, and utilities in functional contexts (onboarding, settings, auth).
- Inter 400 · 13 · lowercase · color `textSecondary` · no `textDecorationLine`; the underline is a hairline drawn below: wrapper with `borderBottomWidth: 1`, `borderBottomColor: rgba(255,255,255,0.28)`, `paddingBottom: 3`, `alignSelf: 'flex-start'`.
- Press state: hairline brightens to 0.5; text unchanged.
- Examples: `not now`, `skip · add later`, `already keeping a field? sign in`.

### 6e. Classification rule (apply in order) + non-members
1. The action the surface exists for, at full ceremony → **CTA** (max one per surface).
2. The action of this screen/moment → **primary**.
3. A doorway offered inside a held space → **whisper**.
4. Any other tappable text → **secondary**.
Non-members: the one-time map hint is the `micro` text register (the map itself is the target); eyebrows are never tappable; the TODAY pendulum chip on Origin is a chip, styled as built.
Migration: classify every existing tappable text and list assignments in the checkpoint summary for founder veto.

### 6f. Added acceptance
- [ ] `components/Links.tsx` exists with the three link species; grep finds no inline-styled tappable text outside it and the CTA component.
- [ ] No underlines via `textDecorationLine` anywhere; secondary's hairline renders 3px below.
- [ ] Every whisper link hits a ≥ 44pt target.
